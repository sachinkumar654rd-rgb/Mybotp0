...
// --- इनिशियलाइजेशन (Initialization) ---
const firebaseApp = !getApps().length ? initializeApp(firebaseConfig) : getApp();
const db = getFirestore(firebaseApp);
const bot = new Telegraf(BOT_TOKEN);

/**
 * गेम डेटा सिंक करना और फायरबेस में स्टोर करना
 */
async function syncData() {
    try {
        const res = await axios.get(`${API_URL}?pageSize=50&_t=${Date.now()}`);
...
/**
 * 100,000+ रिकॉर्ड्स स्कैन करके प्रेडिक्शन निकालना (L10 -> L3)
 */
async function calculateAIPrediction(currentSequence) {
    ...
    // डिफॉल्ट प्रेडिक्शन अगर कोई मैच न मिले
...
/**
 * ऑटोमेशन: चैनल में पोस्ट करना और रिजल्ट अपडेट करना
 */
async function processAutomation() {
    ...
    // 1. पिछला रिजल्ट अपडेट करें (Win/Loss)
    ...
    if (lastSnap.exists()) {
        ...
        if (data.issueNumber === currentIssue && !data.completed) {
            ...
            } catch (e) { console.log("Edit failed (msg deleted?)"); }
        }
    }

    // 2. नया प्रेडिक्शन भेजें
...
bot.command('history', async (ctx) => {
    ...
    rows.sort((a,b) => b.issueNumber - a.issueNumber).forEach(r => {
        text += `\`${r.issueNumber.slice(-4)}\`: ${r.number} (${r.number >= 5 ? 'B' : 'S'})\n`;
    });
...
setInterval(processAutomation, 30000); // 30 सेकंड में ऑटो सिंक
processAutomation();
bot.launch();
